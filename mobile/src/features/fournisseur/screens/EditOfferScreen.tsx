...
export default function EditOfferScreen() {
  ...
  const [price, setPrice] = useState(offer?.price ? offer.price.toString() : '');
  const [stockQuantity, setStockQuantity] = useState(offer?.stockQuantity ? offer.stockQuantity.toString() : '');
  const [photos, setPhotos] = useState<string[]>(offer?.photos || []);
  ...
  const handleSubmit = async () => {
    ...
    setLoading(true);
    try {
      await api.patch(`/products/${offer.id}`, {
        type,
        title,
    ...
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
      ...
      <View style={styles.header}>
        ...
        <View style={styles.headerCenter}>
          <Icon3D icon="✏️" size={24} bgColor="#7C3AED" />
